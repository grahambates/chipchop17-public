...
void PatchBootBlockCheckSum( unsigned char* buf )
{
    ...
    // Explicitly clear the old checksum area
    buf[4] = buf[5] = buf[6] = buf[7] = 0;

    for(int i = 0; i < 1024; i += 4) {
        // Copy 4 bytes into a uint32_t in a portable, alignment-safe way
        uint32_t bufValue = 0;
        memcpy(&bufValue, buf + i, sizeof(uint32_t));
        
        // Swap bytes if necessary (assuming the target is little-endian)
        d = BSwapInt(bufValue);

        // Check for overflow and adjust sum
        if (UINT32_MAX - newsum < d) {
            newsum++;
        }
    ...
    // Write back the checksum in a portable, alignment-safe way
    uint32_t checksumSwapped = BSwapInt(newsum);
    memcpy(buf + 4, &checksumSwapped, sizeof(uint32_t));
}
...
int main(int argc, char **argv)
{
	...
	// reserve the first 4 sectors of the diskbuffer for bootblock and our directory
	int fileoffset=4*512;
	...
	BSwapIntArray((unsigned int*)(mydirsector), 256);
